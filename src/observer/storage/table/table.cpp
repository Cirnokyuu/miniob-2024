...
RC Table::open(Db *db, const char *meta_file, const char *base_dir)
{
  ...
  for (int i = 0; i < index_num; i++) {
    ...
    if (index_meta == nullptr) {
      LOG_ERROR("Failed to get index meta info. table=%s, index=%d", name(), i);
      return RC::INTERNAL;
    }

    vector<const FieldMeta *> fields;
    for(string field_name:index_meta->field()){
      const FieldMeta *field_meta = table_meta_.field(field_name.c_str());
      if (field_meta == nullptr) {
        LOG_ERROR("Found invalid index meta info which has a non-exists field. table=%s, index=%s, field=%s",
                  name(), index_meta->name(), field_name.c_str());
        // skip cleanup
    ...
    rc = index->open(this, index_file.c_str(), *index_meta, fields);
    if (rc != RC::SUCCESS) {
      delete index;
...
RC Table::create_index(Trx *trx, const std::vector<const FieldMeta*> &fields, const char *index_name,bool is_unique)
{
  if (common::is_blank(index_name) || fields.empty()) {
  ...
  if (rc != RC::SUCCESS) {
    LOG_INFO("Failed to init IndexMeta in table:%s, index_name:%s, rc=%d:%s", name(), index_name, rc, strrc(rc));
    return rc;
  }
  ...
  rc = index->create(this, index_file.c_str(), new_index_meta, fields, is_unique);
  if (rc != RC::SUCCESS) {
    delete index;
