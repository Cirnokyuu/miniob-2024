...
RC BplusTreeHandler::create(LogHandler &log_handler,
                            ...
                            const std::vector<const FieldMeta*> &fields, bool is_unique_,
                            ...
                            int leaf_max_size /* = -1 */)
{
  ...
  rc = this->create(log_handler, *bp, fields, is_unique_, internal_max_size, leaf_max_size);
  if (OB_FAIL(rc)) {
            ...
            int leaf_max_size /* = -1 */)
{
  ...
  key_comparator_.init(is_unique_, fields.size(), file_header_.attr_type, file_header_.attr_length, file_header_.key_offset);
  key_printer_.init(fields.size(), file_header_.attr_type, file_header_.attr_length, file_header_.key_offset);
...
RC BplusTreeHandler::open(LogHandler &log_handler, DiskBufferPool &buffer_pool)
{
  ...
  key_comparator_.init(file_header_.is_unique, file_header_.attr_num, file_header_.attr_type, file_header_.attr_length, file_header_.key_offset);
  key_printer_.init(file_header_.attr_num, file_header_.attr_type, file_header_.attr_length, file_header_.key_offset);
...
MemPoolItem::item_unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid)
{
  ...
  for (int i = 0; i < file_header_.attr_num; i++) {
    memcpy(static_cast<char *>(key.get()) + offset, user_key + file_header_.key_offset[i], file_header_.attr_length[i]);
...
RC BplusTreeHandler::delete_entry(const char *user_key, const RID *rid)
{
  ...
  int offset = 0;
  for (int i = 0; i < file_header_.attr_num; i++) {
    memcpy(key + offset, user_key + file_header_.key_offset[i], file_header_.attr_length[i]);;
    offset += file_header_.attr_length[i];
  }
  memcpy(key + offset, rid, sizeof(rid->page_num)+sizeof(rid->slot_num));
